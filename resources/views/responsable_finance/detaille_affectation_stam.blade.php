...
@section('title', 'Détails Affectation Stam')
...
<div class="container">
    <h1>Détails Affectation Stam</h1>
    ...
    <table id="stamTable" class="table table-bordered table-striped">
        <thead>
            <tr>
                <th>Facture</th>
        ...
        <tbody>
            @foreach($factures as $facture)
                @if($facture->stam_preparer_paiement) <!-- Vérifier si Stam est présent -->
                    <tr>
                        <td>{{ $facture->facture }}</td>
                        <td>{{ $facture->num_conteneur }}</td>
                        <td>{{ $facture->fournisseur }}</td>
                        <td>
                            @if($facture->stam_preparer_paiement)
                                {{ $facture->stam_preparer_paiement }}
                            @else
                        ...
                        <td>
                            @if($facture->statut_finance)
                                {{ $facture->statut_finance }}
                            @else
                                <span class="na-value"><i class="fas fa-exclamation-circle"></i> Pas d'État</span>
                            @endif
                        </td>
                        <td>
                            @if($facture->Attachement_Stam) <!-- Supposons que vous avez un champ pour la pièce jointe -->
                                <a href="{{ asset('storage/' . $facture->Attachement_Stam) }}" target="_blank">
