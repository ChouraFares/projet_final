...
<div class="dashboard">
    <div class="header">
        <div>
            <h2><i class="fas fa-car me-2"></i>Modifier Contrat Flotte</h2>
        </div>
    </div>
    <div class="form-container">
        ...
        @endif
        <form action="{{ route('admin.assurance.flotte.update', $contrat->id) }}" method="POST" enctype="multipart/form-data" id="flotteForm">
            @csrf
            @method('PUT')
            <div class="row">
                <!-- Première colonne -->
                ...
                <div class="card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('objet_avenant') is-invalid @enderror"
                               id="objet_avenant" name="objet_avenant" value="{{ old('objet_avenant', $contrat->objet_avenant) }}" required>
                        @error('objet_avenant')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
...
<script>
    ...
    document.getElementById('flotteForm').addEventListener('submit', function(e) {
        const requiredFields = ['compagnie_assurance', 'ref_contrat', 'date_effet', 'echeance', 'condition_renouvellement', 'avenant', 'objet_avenant'];
        let isValid = true;
