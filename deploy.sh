...
# Vérifier la présence de marqueurs de conflit, en excluant vendor/ et deploy.sh
echo "Vérification des marqueurs de conflit (<<<<<<<)..."
CONFLICT_FILES=$(grep -r "<<<<<<< HEAD" . --exclude-dir=vendor --exclude=deploy.sh 2>/dev/null)
...
# Synchroniser la VM avec GitHub
echo "Synchronisation de la VM avec GitHub..."
ssh administrateur@192.168.80.105 "cd ~/bk_food_new && git fetch && git reset --hard origin/master && php artisan cache:clear && php artisan config:cache" || { echo "❌ Erreur lors du déploiement sur la VM"; exit 1; }

# Vérification finale sur la VM, en excluant vendor/
echo "Vérification finale sur la VM..."
CONFLICT_CHECK=$(ssh administrateur@192.168.80.105 "cd ~/bk_food_new && grep -r '<<<<<<< HEAD' . --exclude-dir=vendor 2>/dev/null")
